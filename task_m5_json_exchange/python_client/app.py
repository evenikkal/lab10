"""
FastAPI service for task M5 — demonstrates Python-side of the
"FastAPI (Python) vs Gin (Go)" comparison.

The service acts as a gateway: it validates incoming orders with Pydantic,
forwards them to the Go Gin service, and returns enriched responses.

Usage:
    uvicorn app:app --port 8090 --reload

The Go service must be running on :8082:
